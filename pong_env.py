...
class PongSinglePlayerEnv(gym.Env):
    ...
    def __init__(self, ball_speed=4, bat_speed=4, max_num_rounds=20):
        ...
        #     np.full((SCREEN_HEIGHT, SCREEN_WIDTH, 3), 256)
        # )
        # self.observation_space = 4
        # self.action_space = 1 #spaces.Discrete(3) # 3 possible actions, in 1 space
        self.observation_space = spaces.Box(
            low=0,
            high=max(SCREEN_HEIGHT, SCREEN_WIDTH),
            shape=(4,))
    ...
    def _get_state(self):
        ...
        return np.array([ballx, bally, left, right])
