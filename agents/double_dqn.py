...
class DoubleDQNAgent(BaseAgent):
    def __init__(self, action_space, observation_space, 
        ...
        self.online_net = DQNNetwork(input_dim, output_dim).to(self.device)
    ...
    def learn(self, reward, next_observation, done):
        ...
        with torch.no_grad():
            next_actions = self.online_net(next_state_batch).max(1)[1].unsqueeze(1)
            next_state_values = self.target_net(next_state_batch).gather(1, next_actions).squeeze(1)
            expected_q_values = reward_batch + (1 - done_batch) * self.gamma * next_state_values
